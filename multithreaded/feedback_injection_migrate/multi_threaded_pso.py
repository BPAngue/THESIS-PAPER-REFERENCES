...
STAGNATION_LIMIT = 10 # Increased limit
...
# --------------------------------------
# System Pipeline Main Loop (MODIFIED)
# --------------------------------------
solvers = [pso_solver.BasePSOSolver(nPop_per_thread, varSize, pso_params) for _ in range(NUM_THREADS)]

# Voted_GBest is now just a *tracker* for the best solution ever found
...
for round_num in range(MAX_VOTING_ROUNDS):
    ...
    # The "voted" best is just the best one from this round
    round_best = gbest_candidates[0]
    
    # --- [START] MODIFIED STOPPING & FEEDBACK ---
    
    # 1. Check for perfect solution
    ...
    # 3. Feedback Phase (Migration, not Broadcasting)
    # We "migrate" the best solutions in a ring.
    # Thread 0's best -> injected into Thread 1
    # Thread 1's best -> injected into Thread 2
    # ...
    # Thread N's best -> injected into Thread 0
    
    print("Migrating best particles between swarms...")
    
    # We use the *full list* of candidates, which are already sorted
    # This ensures we get the gbest from each solver, even if they aren't all unique
    
    # We need to get the actual gbest from each solver *in order*
    current_gbests = [s.get_gbest() for s in solvers]
    
    for i in range(NUM_THREADS):
        # The particle to be injected comes from the *previous* thread
        particle_to_inject = current_gbests[(i - 1 + NUM_THREADS) % NUM_THREADS]
        
        # The particle is injected into the *current* thread
    ...
    # --- [END] MODIFIED STOPPING & FEEDBACK ---
...
# We report the Best-Ever particle found across all rounds
